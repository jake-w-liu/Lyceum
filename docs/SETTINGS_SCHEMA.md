...
run profiles in M8, LaTeX in M11).

---

## 1. Conventions

- Settings live in a single JSON object. `runtimePaths` is a nested object
  because runtime overrides are keyed by language profile.
...
- Empty string (`""`) on a path-valued key means "resolve automatically"
  (default shell / runtime on `PATH`), never "the current directory".
...
|-----|------|---------|----------------|-------------|
| `version` | integer | `2` | positive integer | Reserved schema-version field used for migration. Not user-facing; managed by the app. |
...
| `runtimePaths` | object | see below | string values | Optional executable overrides for built-in run profiles and Julia LSP startup. Empty strings mean "resolve from `PATH`". |
...
| `zoomLevel` | integer | `0` | integer -5 to 10 | VS Code-style whole-window zoom level. `0` means 100%; each step maps to the app's native webview zoom factor. |

`runtimePaths` has this shape:

| Key | Default | Used for |
|-----|---------|----------|
| `runtimePaths.julia` | `""` | Julia run profile, Julia REPL, and Julia LanguageServer.jl startup. |
| `runtimePaths.python` | `""` | Python run profile and Python REPL. |
| `runtimePaths.node` | `""` | JavaScript run profile. |
| `runtimePaths.shell` | `""` | Shell-script run profile. Falls back to `shellPath`, then `sh`. |
| `runtimePaths.r` | `""` | R run profile. |
...
This is the complete default settings object. A fresh install starts with these
in-memory defaults and writes the file the first time settings are saved.
...
field. The current settings schema version is **`2`**. The current keybindings
schema version remains **`1`**.

Loading algorithm:

1. Read and parse the file. On parse failure, keep in-memory defaults and leave
...
- `1 -> 2`: read deprecated top-level `juliaPath` into `runtimePaths.julia`
