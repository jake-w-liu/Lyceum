...
| `latexBuildCommand` | string | `"latexmk -pdf main.tex"` | any shell command string | Command template run for the Markdown/LaTeX build-and-preview workflow (M11). With a `.tex` tab active, Lyceum retargets the final `.tex` argument to that file and runs in the file's directory; otherwise it runs as configured from the workspace root. When the stock default is unchanged, Lyceum auto-selects the first installed engine from `latexmk`, `tectonic`, `pdflatex`, `xelatex`, and `lualatex`. Custom commands are run exactly as configured; use an absolute compiler path if your TeX tools are installed outside standard macOS GUI-app paths. |
